...
def print_banner():
    """打印平台横幅"""
    logger.info("\n" + "="*80)
    logger.info(" "*15 + "101因子分析平台 - QMT集成版 v1.0")
    logger.info("="*80)
    logger.info("功能列表：")
    logger.info("  1. IC/IR分析     - 评估因子预测能力")
    logger.info("  2. 相关性分析    - 识别重复因子")
    logger.info("  3. 分层回测      - 验证因子有效性")
    logger.info("  4. 完整分析      - 一键执行所有分析")
    logger.info("  5. 从QMT获取数据 - 获取行情并计算基础因子")
    logger.info("  0. 退出平台")
    logger.info("="*80 + "\n")
...
def load_data_from_qmt():
    ...
    try:
        ...
        for i, code in enumerate(stock_codes[:100]):  # 限制前100只股票演示
            try:
                logger.info(f"\r获取进度: {i+1}/{min(len(stock_codes), 100)}", end='')
        ...
        logger.info(f"\n[OK] 数据准备完成！")
        logger.info(f"  日期范围: {price_data.index[0]} 至 {price_data.index[-1]}")
        logger.info(f"  股票数量: {len(price_data.columns)}")
        ...
        logger.info("\n[错误] 无法导入easy_xt模块")
        logger.info("请确保已安装easy_xt: pip install -e easy_xt")
        return None
    except Exception as e:
        logger.info(f"\n[错误] 从QMT获取数据失败: {str(e)}")
        import traceback
...
def load_data_example():
    ...
    logger.info(f"[OK] 示例数据生成完成：{len(dates)}个交易日，{len(stocks)}只股票，{len(factor_dict)}个因子\n")
...
def load_data_from_file():
    ...
    logger.info("="*80)
    logger.info("\n请准备数据文件：")
    logger.info("  - 价格数据：CSV格式，索引为日期，列为股票代码")
    logger.info("  - 因子数据：CSV格式，索引为日期，列为股票代码")
    ...
    for factor_name, factor_data in selected_factors.items():
        ...
        try:
            ...
            if save == 'y':
                ...
                logger.info(f"[OK] 结果已保存到 {REPORT_DIR}\n")

        except Exception as e:
            logger.info(f"[X] 分析失败：{str(e)}\n")
...
def layered_backtest(price_data, factor_dict):
    ...
    for factor_name, factor_data in selected_factors.items():
        ...
        try:
            ...
            logger.info(f"[X] 回测失败：{str(e)}\n")


def complete_analysis(price_data, factor_dict):
    """完整分析"""
    logger.info("\n" + "="*80)
    logger.info("完整因子分析流程")
    logger.info("="*80)

    logger.info("\n将对所有因子进行系统分析...")
    logger.info("分析流程：IC/IR分析 -> 分层回测 -> 因子排名\n")

    results = {}

    for factor_name, factor_data in factor_dict.items():
        logger.info(f"\n{'='*80}")
        logger.info(f"正在分析: {factor_name}")
        logger.info(f"{'='*80}")
        ...
        try:
            ...
            logger.info(f"[OK] {factor_name} 分析完成")

        except Exception as e:
            logger.info(f"[X] {factor_name} 分析失败: {str(e)}")
            continue

    # 生成综合报告
    logger.info(f"\n{'='*80}")
    logger.info("因子综合分析报告")
    logger.info(f"{'='*80}\n")

    results_df = pd.DataFrame(results).T
    results_df = results_df.sort_values('IR', ascending=False)

    logger.info("因子排名（按IR排序）：\n")
...
def main():
    ...
    while True:
        ...
        # 加载数据
        logger.info("请选择数据来源：")
        logger.info("  1. 从QMT获取数据（推荐）")
        logger.info("  2. 从文件加载")
        logger.info("  3. 使用示例数据（快速体验）")
