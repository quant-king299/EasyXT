...
class LayeredBacktester:
    ...
    def calculate_returns(
        ...
        """
        if periods <= 0:
            raise ValueError("periods 必须大于0")
    ...
    def create_layers(
        ...
        if n_layers < 2:
            raise ValueError("n_layers 必须至少为2")
    ...
    def calculate_layer_returns(
        ...
        for date in self.layer_positions.keys():
            ...
            layer_return_dates.append(date)

        self.layer_returns = pd.DataFrame(
            layer_returns_list, index=pd.Index(layer_return_dates, name='date'))
    ...
    def calculate_backtest_metrics(
        ...
        annual_return = annualized_return(
            total_return, n_periods, annual_days=annualization_factor)

        # 夏普比率
        sharpe_ratio = annualized_sharpe(
            returns,
            risk_free_rate=risk_free_rate,
            annual_days=annualization_factor,
        )
    ...
    def print_report(self):
        ...
        if self.layer_returns is not None:
            ...
            logger.info(layer_stats.to_string(index=False))
