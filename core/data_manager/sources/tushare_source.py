...
class TushareSource(BaseDataSource):
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if not self.tokens:
                logger.info("[TushareSource] 未提供Tushare Token")
                return False
            ...
            for i, token in enumerate(self.tokens):
                try:
                    ...
                    logger.info(f"[TushareSource] {label}连接成功")
                    return True
                except Exception as e:
                    label = f"主token" if i == 0 else f"备用token({i})"
                    logger.info(f"[TushareSource] {label}连接失败: {e}")
                    continue

            logger.info("[TushareSource] 所有token均连接失败")
            self._connection = None
            self.is_connected = False
            return False

        except ImportError:
            logger.info("[TushareSource] tushare模块未安装")
            return False
    ...
    def _switch_token(self) -> bool:
        ...
        try:
            ...
            logger.info(f"[TushareSource] 已切换到{label}")
            return True
        except Exception as e:
            logger.info(f"[TushareSource] 切换token失败: {e}")
            return False
        ...
        for attempt in range(len(self.tokens)):
            try:
                ...
                if self._is_rate_limited(e):
                    logger.info(f"[TushareSource] 请求被限流，尝试切换token...")
    ...
    def get_price(self,
        ...
        try:
            ...
            if not validate_date(start_date) or not validate_date(end_date):
                logger.info(f"[TushareSource] 日期格式错误: {start_date} - {end_date}")
                return None
            ...
            logger.info(f"[TushareSource] 获取价格数据失败 ({symbol}): {e}")
            return None
    ...
    def get_fundamentals(self,
        ...
        try:
            ...
            if not validate_date(date):
                logger.info(f"[TushareSource] 日期格式错误: {date}")
                return None
                ...
                for i in range(1, 4):
                    try:
                        ...
                        if df is not None and not df.empty:
                            logger.info(f"[TushareSource] {date}无数据，使用{prev_date}数据")
                            break
                    except (ValueError, TypeError):
                        continue

            if df is None or df.empty:
                logger.info(f"[TushareSource] 获取基本面数据失败：{date}无数据")
                return None
            ...
            logger.info(f"[TushareSource] 获取基本面数据失败: {e}")
            return None
    ...
    def get_trading_dates(self,
        ...
        try:
            ...
            logger.info(f"[TushareSource] 获取交易日历失败: {e}")
            return None
    ...
    def get_stock_list(self,
        ...
        try:
            ...
            logger.info(f"[TushareSource] 获取股票列表失败: {e}")
            return None
    ...
    def get_index_list(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            logger.info(f"[TushareSource] 获取指数列表失败: {e}")
            return None
