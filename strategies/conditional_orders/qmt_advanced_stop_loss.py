...
def simulate_strategy():
    ...
    if strategy.trade_records:
        ...
        for record in strategy.trade_records:
            ...
            logger.info(f"时间: {record['time']}")
            logger.info(f"股票: {record['stock_code']}")
            logger.info(f"操作: {record['action']} {record['quantity']}股 @{record['price']:.2f}")
            logger.info(f"盈亏: {record['profit_loss']*100:+.2f}%")
            logger.info(f"触发策略: {[s['strategy_id'] for s in record['triggered_strategies']]}")
...
def main():
    ...
    logger.info("="*60)
    logger.info("🎯 QMT高级止盈止损策略")
    logger.info("基于条件单的四重止盈止损体系")
    logger.info("="*60)
    ...
    logger.info("\n💡 使用说明:")
    logger.info("1. 选择合适的策略组合（中长线稳健/短线激进/新手保守）")
    logger.info("2. 设置成本价和持仓数量")
    logger.info("3. 选择启用的策略（可以启用全部或部分）")
    logger.info("4. 策略会自动监控并在触发时执行止损")
    logger.info("5. 支持自定义参数覆盖预设值")
