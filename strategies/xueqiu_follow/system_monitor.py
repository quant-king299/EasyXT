...
class SystemMonitor:
    ...
    def _display_status(self):
        ...
        logger.info(f"运行时间: {self._format_timedelta(uptime)}")
        logger.info(f"最后更新: {self.stats['last_update'].strftime('%H:%M:%S') if self.stats['last_update'] else 'N/A'}")
        print()
        ...
        if self.stats['cpu_history']:
            ...
            logger.info(f"   CPU使用率: {latest_cpu:.1f}% {cpu_trend}")
        ...
        if self.stats['memory_history']:
            ...
            logger.info(f"   内存使用率: {latest_memory['percent']:.1f}% {memory_trend}")
            logger.info(f"   可用内存: {latest_memory['available_gb']:.2f}GB")
            logger.info(f"   已用内存: {latest_memory['used_gb']:.2f}GB")
        ...
        logger.info("📊 业务指标:")
        logger.info(f"   交易次数: {self.stats['trade_count']}")
        logger.info(f"   错误次数: {self.stats['error_count']}")
        logger.info(f"   警告次数: {self.stats['warning_count']}")
        print()
        ...
        if len(self.stats['network_history']) >= 2:
            ...
            logger.info("🌐 网络状态:")
            logger.info(f"   上传速率: {self._format_bytes(sent_rate)}/s")
            logger.info(f"   下载速率: {self._format_bytes(recv_rate)}/s")
            print()
        ...
        logger.info(f"   启用组合: {len(enabled_portfolios)}")
        

        emergency_stop = self.config_manager.get_global_setting('emergency_stop', False)

        logger.info(f"   紧急停止: {'是' if emergency_stop else '否'}")
        print()
    ...
    def _draw_simple_chart(self, values: List[float], max_val: float = 100, width: int = 40):
        ...
        logger.info(f"   {chart_line}")
        logger.info(f"   0%{' ' * (width-6)}100%")
