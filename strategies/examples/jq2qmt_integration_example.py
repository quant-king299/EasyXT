...
def demo_jq2qmt_integration():
    ...
    logger.info(f"JQ2QMT集成状态: {'已启用' if strategy.enable_jq2qmt else '已禁用'}")
    ...
    if strategy.enable_jq2qmt:
        ...
        logger.info(f"JQ2QMT连接状态: {sync_status}")
        ...
        logger.info(f"JQ2QMT持仓数量: {len(jq2qmt_positions)}")
        ...
        if diff_result:
            ...
            logger.info(f"  需要买入: {len(diff_result.get('to_buy', []))}")
            logger.info(f"  需要卖出: {len(diff_result.get('to_sell', []))}")
            logger.info(f"  需要调整: {len(diff_result.get('to_adjust', []))}")
            logger.info(f"  无需变动: {len(diff_result.get('unchanged', []))}")
        ...
        logger.info(f"一键同步结果: {submit_result}")
        ...
        logger.info(f"qka连接测试: {'OK' if ok else 'FAIL'}")
    

    else:

        logger.info("JQ2QMT集成未启用，请检查配置")
