...
class SimpleFunctionAdapter(StrategyBase):
    ...
    def _load_category_data(self):
        ...
        if self.category not in ('cb', 'etf'):
            print(f"[INFO] SimpleFunctionAdapter: stock 类别不预加载，按需查询 DuckDB")
            return
        ...
        try:
            ...
            if not df.empty:
                ...
                print(f"[WARN] SimpleFunctionAdapter: {self.category} 数据为空")

            self._category_data = df
        except Exception as e:
            print(f"[ERROR] SimpleFunctionAdapter 预加载失败: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _query_stock_prices_for_date(self, symbols: List[str], date: str) -> Dict[str, float]:
        ...
        try:
            ...
            print(f"  [DEBUG] _query_stock_prices_for_date 异常: {e}")
            return {}
        finally:
            if con is not None:
                try:
    ...
    def _query_stock_price(self, symbol: str, date: str) -> Optional[float]:
        ...
        try:
            ...
            print(f"  [DEBUG] _query_stock_price 异常: symbol={symbol}, date={date}, error={e}")
            return None
        finally:
            if con is not None:
                try:
    ...
    def _query_stock_prices_batch(self, symbols: List[str],
        ...
        try:
            ...
            print(f"[WARN] _query_stock_prices_batch 失败: {e}")
        finally:
            if con is not None:
                try:
                    con.close()
    ...
    def select_stocks(self, date: str) -> List[str]:
        ...
        if self._category_data is None or self._category_data.empty:
            print(f"  [DEBUG] select_stocks: 缓存数据为空 [类别:{self.category}, 日期:{date}]")
            return []

        date_dt = self._date_to_ts(date)
        day_df = self._category_data[self._category_data['trade_date'] == date_dt].copy()
        if day_df.empty:
            print(f"  [DEBUG] select_stocks: 当日无数据 [类别:{self.category}, 日期:{date}]")
            return []

        print(f"  [DEBUG] select_stocks: 输入数据形状={day_df.shape}, 类别={self.category}")
        sample_codes = day_df['ts_code'].head(3).tolist()
        print(f"  [DEBUG] select_stocks: 输入数据中的代码示例: {sample_codes}")

        try:
            result = self.func(day_df, top_n=self.top_n, **self._extra)
        except Exception as e:
            print(f"  [ERROR] select_stocks: 策略函数执行失败: {e}")
            return []
    ...
    def _select_stocks_from_duckdb(self, date: str) -> List[str]:
        ...
        try:
            ...
            if df.empty:
                print(f"  [DEBUG] select_stocks(stock): 当日无数据 [日期:{date}]")
                return []
            df['trade_date'] = pd.to_datetime(df['trade_date'])

            print(f"  [DEBUG] select_stocks(stock): 输入数据形状={df.shape}")
            sample_codes = df['ts_code'].head(3).tolist()
            print(f"  [DEBUG] select_stocks(stock): 输入数据中的代码示例: {sample_codes}")
            ...
            print(f"  [ERROR] _select_stocks_from_duckdb 失败: {e}")
            return []
        finally:
            if con is not None:
                try:
