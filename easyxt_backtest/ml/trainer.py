...
class DuckDBModelTrainer:
    ...
    def compute_labels(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        print(f"[LABEL] 构造标签 (T+{self.label_horizon})...")
        df = df.sort_values(["stock_code", "date"]).copy()
    ...
    def train(
        ...
        """
        print("=" * 60)
        print("DuckDB LightGBM 训练")
        print("=" * 60)
        print(f"训练集: {train_start} ~ {train_end}")
        print(f"验证集: {valid_start} ~ {valid_end}")
        print(f"标签:   未来 {self.label_horizon} 日收益")
        print(f"模型:   LightGBM (n_est={self.n_estimators}, lr={self.learning_rate})")
        ...
        # 5. 评估
        print(f"[5/5] 评估 (训练耗时 {elapsed:.0f}s)")
    ...
    def load(self, path: str):
        ...
        print(f"  特征: {len(self._feature_names)} 维")
        if self._metrics:
            print(f"  IC: {self._metrics.get('IC', 'N/A')}")
