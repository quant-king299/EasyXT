...
class EnhancedBacktestResult:
    ...
    def print_summary(self):
        ...
        logger.info(f"  初始资金:     {perf.get('initial_cash', 0):,.2f}")
        logger.info(f"  最终资金:     {perf.get('final_value', 0):,.2f}")
        ...
        logger.info(f"\n【交易统计】")
        logger.info(f"  总交易次数:   {len(self.trades) if not self.trades.empty else 0}")
        logger.info(f"  交易天数:     {len(self.portfolio_history) if not self.portfolio_history.empty else 0}")
...
class EnhancedBacktestEngine:
    ...
    def run_backtest(self,
        ...
        logger.info(f"\n{'='*80}")
        logger.info(f"  增强回测引擎")
        logger.info(f"{'='*80}")
        logger.info(f"  策略: {strategy.name if hasattr(strategy, 'name') else 'Unknown'}")
        logger.info(f"  期间: {start_date} - {end_date}")
        logger.info(f"  初始资金: {self.initial_cash:,.2f}")
        logger.info(f"  佣金: {self.commission:.2%}")
        logger.info(f"  复权: {self.adjust}")
        ...
        if not trading_dates:
            ...
            logger.warning("[WARN] 无法获取交易日历")
            return self._empty_result()



        logger.info(f"  [OK] 获取交易日: {len(trading_dates)} 个")
        ...
        if len(sorted_data_dates) < 10:
            ...
            logger.warning(f"[WARN] 数据日期不足({len(sorted_data_dates)})，回退使用交易日历")
    ...
    def _load_daily_prices(self, symbols: List[str],
        ...
        try:
            ...
            logger.info(f"[INFO] 批量加载 {len(symbols)} 只股票的日线数据...")
            ...
            logger.info(f"[OK] 成功加载 {len(all_price_data)} 只股票的日线数据")


        except Exception as e:

            logger.warning(f"[WARN] 批量加载日线数据失败: {e}")
    ...
    def _print_results(self, statistics: Dict) -> None:
        ...
        if not statistics:
            ...
            logger.warning("[WARN] 无统计数据")
            return



        logger.info(f"\n【时间范围】")
        logger.info(f"  起始日期: {statistics.get('start_date', 'N/A')}")
        logger.info(f"  结束日期: {statistics.get('end_date', 'N/A')}")
        logger.info(f"  总交易日: {statistics.get('total_days', 0)}")


        logger.info(f"\n【资金情况】")
        logger.info(f"  初始资金: {statistics.get('initial_cash', 0):,.2f}")
        logger.info(f"  结束资金: {statistics.get('end_balance', 0):,.2f}")
        logger.info(f"  总盈亏: {statistics.get('total_net_pnl', 0):,.2f}")


        logger.info(f"\n【收益指标】")
        logger.info(f"  总收益率: {statistics.get('total_return', 0):.2f}%")
        logger.info(f"  年化收益: {statistics.get('annual_return', 0):.2f}%")
        logger.info(f"  最大回撤: {statistics.get('max_drawdown', 0):,.2f}")
        logger.info(f"  最大回撤率: {statistics.get('max_ddpercent', 0):.2f}%")
        logger.info(f"  最大回撤持续: {statistics.get('max_drawdown_duration', 0)} 天")


        logger.info(f"\n【交易统计】")
        logger.info(f"  盈利天数: {statistics.get('profit_days', 0)}")
        logger.info(f"  亏损天数: {statistics.get('loss_days', 0)}")
        logger.info(f"  总成交额: {statistics.get('total_turnover', 0):,.2f}")
        logger.info(f"  总手续费: {statistics.get('total_commission', 0):,.2f}")
        logger.info(f"  总成交笔数: {statistics.get('total_trade_count', 0)}")


        logger.info(f"\n【风险指标】")
        logger.info(f"  夏普比率: {statistics.get('sharpe_ratio', 0):.2f}")
        logger.info(f"  收益回撤比: {statistics.get('return_drawdown_ratio', 0):.2f}")
