...
class SmartDataDetector:
    ...
    def print_missing_report(self, report: Dict):
        ...
        print()
        logger.info("=" * 60)
        logger.info(f"数据缺失报告: {report['stock_code']}")
        logger.info("=" * 60)
        logger.info(f"检查范围: {report['check_range'][0]} ~ {report['check_range'][1]}")
        logger.info(f"应有交易日: {report['expected_trading_days']} 天")
        print()
        ...
        logger.info("现有数据:")
        if existing['count'] > 0:
            logger.info(f"  数据量: {existing['count']} 条")
            logger.info(f"  日期范围: {existing['first_date']} ~ {existing['last_date']}")
        else:
            logger.info("  无数据")
        print()
        ...
        logger.info(f"缺失交易日: {len(missing)} 天")
        logger.info(f"数据完整度: {report['completeness_ratio']*100:.2f}%")
        print()

        if report['missing_segments']:
            logger.info("缺失数据段:")
            for i, segment in enumerate(report['missing_segments'], 1):
                logger.info(f"  段 {i}: {segment['start']} ~ {segment['end']} ({segment['days']} 天)")
        else:
...
def test_smart_detection():
    ...
    # 测试单个股票
    logger.info("[1] 检测 511380.SH 数据缺失...")
    ...
    logger.info(f"总标的数: {plan['total_stocks']}")
    logger.info(f"有缺失的标的: {plan['stocks_with_missing_data']}")
    logger.info(f"总缺失天数: {plan['total_missing_days']}")
    print()

    # 打印下载任务
    logger.info("下载任务:")
    for task in plan['download_tasks']:
        logger.info(f"  {task['stock_code']}: {len(task['segments'])} 个缺失段")
