...
class UnifiedDataInterface:
    ...
    def connect(self, read_only: bool = False):
        ...
        try:
            ...
            logger.info("[INFO] DuckDB 连接成功")
            return True
        except Exception as e:
            logger.error(f"[ERROR] DuckDB 连接失败: {e}")
            self.con = None
            return False
    ...
    def get_stock_data(
        ...
        if self.duckdb_available and self.con:
            logger.info(f"  [INFO] 使用QMT API复权方案查询数据 [股票:{stock_code}]")
            ...
            try:
                ...
                if period == '1d':
                    ...
                    if data is not None and not data.empty:
                        logger.info(f"  [OK] 从DuckDB获取成功 {len(data)} 条记录 [股票:{stock_code}]")
                        # 验证stock_code列是否存在且正确
                        if 'stock_code' in data.columns and not data.empty:
                            actual_codes = data['stock_code'].unique()
                            logger.debug(f"  [DEBUG] 返回数据中的stock_code: {actual_codes}")
                    else:
                        data = None
                else:
                    # 分钟线数据不需要复权，直接查询
                ...
                logger.warning(f"  [WARN] 复权缓存查询失败: {e}")
                logger.info(f"  [INFO] 降级到原有的_read_from_duckdb方法")
        ...
        if data is None or data.empty:
            logger.info(f"  → DuckDB 无数据，需要从在线数据源获取")
            need_download = True
        else:
            # 检查是否有缺失
        ...
        if need_download:
            ...
            if auto_save and self.duckdb_available and self.con:
                logger.info(f"  → 保存数据到 DuckDB...")
                self._save_to_duckdb(data, stock_code, period)
                logger.info(f"  [OK] 数据已保存到 DuckDB")
        else:
            logger.info(f"  [OK] 从 DuckDB 读取成功（{len(data)} 条记录）")
    ...
    def _read_from_duckdb(
        ...
        try:
            ...
            if not table_exists:
                logger.info(f"  [INFO] 表 {table_name} 不存在，返回空数据")
                return None
    ...
    def _get_dividends_from_qmt(
        ...
        try:
            ...
            if divid_data is None or divid_data.empty:
                logger.info(f"    [INFO] 无分红数据: {stock_code}")
                return pd.DataFrame()
            ...
            if isinstance(divid_data, pd.DataFrame):
                ...
                if not dividends_df.empty and 'ex_date' in dividends_df.columns:
                    ...
                    logger.info(f"    [OK] 获取 {len(dividends_df)} 条分红记录")
                    return dividends_df
                else:
                    logger.warning(f"    [WARN] 分红数据格式不符，无法使用")
                    return pd.DataFrame()

            return pd.DataFrame()

        except Exception as e:
            logger.warning(f"    [WARN] 获取分红数据失败: {e}")
            return pd.DataFrame()
    ...
    def _merge_data(
        ...
        def ensure_datetime_index(df, name=""):
            ...
            # 如果已经有索引，确保是datetime类型
                ...
                try:
                    ...
                    logger.warning(f"  [WARN] 无法转换{name}的索引类型: {type(df.index)}")
            else:
                # 已经是DatetimeIndex，确保有名字
                if df.index.name is None:
                    df.index.name = 'date'

            logger.debug(f"      [DEBUG] {name}: {len(df)}条, 索引类型={type(df.index).__name__}, 索引名={df.index.name}")
            return df

        # 标准化索引
        logger.debug(f"    [DEBUG] 标准化索引...")
        duckdb_data = ensure_datetime_index(duckdb_data, "DuckDB数据")
        qmt_data = ensure_datetime_index(qmt_data, "QMT数据")

        # 使用QMT数据作为基础
        merged = qmt_data.copy()
        logger.debug(f"    [DEBUG] 复制QMT数据作为基础: {len(merged)}条")

        # 找出DuckDB中有但QMT中没有的日期（用DuckDB补充）
        duckdb_dates = set(duckdb_data.index)
        qmt_dates = set(qmt_data.index)

        logger.debug(f"    [DEBUG] DuckDB日期数: {len(duckdb_dates)}, QMT日期数: {len(qmt_dates)}")

        only_in_duckdb = duckdb_dates - qmt_dates
        logger.debug(f"    [DEBUG] 仅在DuckDB中的日期数: {len(only_in_duckdb)}")

        if only_in_duckdb:
            # 使用.loc[]选择行
            logger.debug(f"    [DEBUG] 添加 {len(only_in_duckdb)} 条DuckDB独有数据...")
            additional = duckdb_data.loc[list(only_in_duckdb)]
            merged = pd.concat([merged, additional]).sort_index()
            logger.debug(f"    [DEBUG] 合并后总计: {len(merged)}条")
        ...
        if before_dedup != after_dedup:
            logger.debug(f"    [DEBUG] 删除 {before_dedup - after_dedup} 条重复记录")

        logger.info(f"    [OK] _merge_data 完成，返回 {len(merged)} 条记录")
        return merged
    ...
    def _save_to_duckdb(
        ...
        try:
            ...
            with self.db_manager.get_write_connection() as con:
                ...
                logger.debug(f"    [DEBUG] reset_index后列名: {list(df_to_save.columns)[:5]}...")
                logger.debug(f"    [DEBUG] 保存数据日期范围: {df_to_save['date'].min()} ~ {df_to_save['date'].max()}")
                ...
                if 'stock_code' not in df_to_save.columns:
                    ...
                    if df_to_save['stock_code'].isna().any() or (df_to_save['stock_code'] == 'None').any():
                        logger.warning(f"    [WARN] 发现stock_code列包含None值，将替换为: {stock_code}")
                        df_to_save['stock_code'] = stock_code
                ...
                logger.debug(f"    [DEBUG] 准备插入 {len(df_ordered)} 条记录...")
                ...
                try:
                    ...
                    logger.info(f"    → 已保存 {len(df_ordered)} 条记录到 {table_name}")
                ...
                logger.debug(f"    [DEBUG] 验证保存结果（{date_min} ~ {date_max}）: count={verify_dict['count']}, max_date={verify_dict['max_date']}")
...
if __name__ == "__main__":
    ...
    if not data2.empty:
        logger.info(f"\n[OK] 便捷函数测试成功")
        logger.info(f"  获取 {len(data2)} 条记录")
