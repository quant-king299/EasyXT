...
class BoardStocksLoader:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.xtdata = None
            logger.error("[ERROR] QMT xtdata 不可用")
    ...
    def get_board_stocks(self, board_name: str) -> List[str]:
        ...
        if not self.available:
            logger.error("[ERROR] QMT 不可用，无法获取板块股票")
            return []
        ...
        if not board_code:
            logger.error(f"[ERROR] 未知板块: {board_name}")
            return []

        logger.info(f"[板块加载] 获取 {board_code} 股票列表...")
        ...
        try:
            ...
            logger.error(f"[ERROR] 获取板块股票失败: {e}")
            return []
    ...
    def _get_all_a_shares(self) -> List[str]:
        ...
        try:
            ...
            if cn_stocks:
                ...
                logger.info(f"[板块加载] 获取沪深A股: {len(cn_stocks)} 只")
            else:
                # 备选方案：分别获取上证A股和深证A股
                logger.info("[板块加载] '沪深A股' 获取失败，尝试分别获取...")
                ...
                if sh_stocks:
                    ...
                    logger.info(f"[板块加载] 获取上证A股: {len(sh_stocks)} 只")
                if sz_stocks:
                    all_stocks.extend(sz_stocks)
                    logger.info(f"[板块加载] 获取深证A股: {len(sz_stocks)} 只")
            ...
            for board_name in bj_board_names:
                try:
                    ...
                    if stocks:
                        ...
                        logger.info(f"[板块加载] 使用 '{board_name}' 获取北交所: {len(stocks)} 只")
                        break
            ...
            if bj_stocks:
                ...
                logger.warning("[WARNING] 未获取到北交所股票，可能需要手动指定板块名称")
            ...
            logger.info(f"[板块加载] 过滤后A股总数: {len(filtered_stocks)} 只")
            return filtered_stocks

        except Exception as e:
            logger.error(f"[ERROR] 获取全A股失败: {e}")
            return []
    ...
    def _get_index_stocks(self, index_code: str) -> List[str]:
        ...
        try:
            ...
            if stocks:
                ...
                logger.warning(f"[WARNING] 未获取到 {index_code} 的成分股")
                return []
    ...
    def _get_market_board(self, board_code: str) -> List[str]:
        ...
        try:
            ...
            logger.error(f"[ERROR] 获取市场板块失败: {e}")
            return []
    ...
    def load_from_csv(self, csv_path: str) -> List[str]:
        ...
        try:
            ...
            logger.info(f"[CSV加载] 从 {csv_path} 加载 {len(cleaned_stocks)} 只股票")
...
if __name__ == "__main__":
    ...
    for csv_file in csv_files:
        if os.path.exists(csv_file):
            ...
            if stocks:
                logger.info(f"加载成功: {stocks[:5]}...")
                break
    else:
        logger.info("未找到CSV文件，跳过测试")
