...
class MoneyFlowAnalyzer:
    ...
    def get_ths_industry_money_flow(self, top_n: int = 20,
        ...
        if use_cache and duckdb_reader is not None and duckdb_reader.conn is not None:
            try:
                ...
                if not check_table.empty:
                    ...
                    if not result.empty:
                        logger.info(f"[OK] 从DuckDB缓存读取行业资金流向数据")
                        data_json = result.iloc[0]['raw_data']
                        df = pd.DataFrame(json.loads(data_json))
                        return df.head(top_n)
            except Exception as e:
                logger.info(f"[INFO] DuckDB缓存读取失败: {e}，从qstock获取...")
        ...
        try:
            ...
            if df.empty:
                logger.info("[INFO] 行业资金流向数据为空")
                return pd.DataFrame()

            logger.info(f"[OK] 从qstock下载行业资金流向: {len(df)} 个行业")
            ...
            if use_cache and duckdb_reader is not None and duckdb_reader.conn is not None:
                try:
                    ...
                    logger.info(f"[OK] 已保存到DuckDB")
                except Exception as e:
                    logger.warning(f"[WARNING] 保存到DuckDB失败: {e}")
    ...
    def get_ths_concept_money_flow(self, top_n: int = 20,
        ...
        if use_cache and duckdb_reader is not None and duckdb_reader.conn is not None:
            try:
                ...
                if not check_table.empty:
                    ...
                    if not result.empty:
                        logger.info(f"[OK] 从DuckDB缓存读取概念资金流向数据")
                        data_json = result.iloc[0]['raw_data']
                        df = pd.DataFrame(json.loads(data_json))
                        return df.head(top_n)
            except Exception as e:
        ...
        try:
            ...
            if df.empty:
                logger.info("[INFO] 概念资金流向数据为空")
                return pd.DataFrame()

            logger.info(f"[OK] 从qstock下载概念资金流向: {len(df)} 个概念")
    ...
    def update_ths_money_flow(self, duckdb_reader=None) -> Dict[str, int]:
        ...
        try:
            ...
            if not df_industry.empty:
                logger.info(f"[OK] 下载成功！共 {len(df_industry)} 个行业")
                ...
                if duckdb_reader is not None and duckdb_reader.conn is not None:
                    ...
                    # 显示TOP10
                    logger.info("\n行业资金流向TOP10：")
                    logger.info(df_industry.head(10))
                else:
                    logger.warning("[WARNING] 未提供DuckDB连接，数据未保存")
            else:
                logger.info("[INFO] 行业资金流向数据为空")

        except Exception as e:
            logger.error(f"[ERROR] 更新行业资金流向失败: {e}")
            import traceback
            traceback.print_exc()
        ...
        try:
            ...
            if not df_concept.empty:
                logger.info(f"[OK] 下载成功！共 {len(df_concept)} 个概念")
                ...
                if duckdb_reader is not None and duckdb_reader.conn is not None:
                    ...
                    # 显示TOP10
                    logger.info("\n概念资金流向TOP10：")
                    logger.info(df_concept.head(10))
                else:
                    logger.warning("[WARNING] 未提供DuckDB连接，数据未保存")
            else:
                logger.info("[INFO] 概念资金流向数据为空")
            ...
            logger.error(f"[ERROR] 更新概念资金流向失败: {e}")
            import traceback
            traceback.print_exc()
    ...
    def get_north_money_flow(self, days: int = 30,
            ...
            try:
                ...
                if not check_table.empty:
                    ...
                    if not df_cached.empty and len(df_cached) >= days:
                        logger.info(f"[OK] 从DuckDB缓存读取北向资金流向: {len(df_cached)} 条")
        ...
        try:
            ...
            if df.empty:
                logger.info("[INFO] 北向资金流向数据为空")
                return pd.DataFrame()

            logger.info(f"[OK] 从qstock下载北向资金流向: {len(df)} 条记录")
    ...
    def get_north_money_sector(self, top_n: int = 20) -> pd.DataFrame:
        ...
        try:
            ...
            if df.empty:
                logger.info("[INFO] 北向资金行业流向数据为空")
                return pd.DataFrame()

            logger.info(f"[OK] 获取北向资金行业流向: {len(df)} 个行业")
    ...
    def get_north_money_stock(self, stock_code: str = None, top_n: int = 20) -> pd.DataFrame:
        ...
        try:
            ...
            if df.empty:
                logger.info("[INFO] 北向资金个股流向数据为空")
                return pd.DataFrame()

            logger.info(f"[OK] 获取北向资金个股流向: {len(df)} 只股票")
    ...
    def get_ths_stock_money_flow(self, stock_code: str = None,
        ...
        if stock_code is None:
            ...
            if use_cache and duckdb_reader is not None and duckdb_reader.conn is not None:
                try:
                    ...
                    if not check_table.empty:
                        ...
                        if not result.empty:
                            logger.info(f"[OK] 从DuckDB缓存读取个股资金流向数据")
                            import json
                            data_json = result.iloc[0]['raw_data']
                            df = pd.DataFrame(json.loads(data_json))
                            return df.head(top_n)
            ...
            try:
                ...
                if df.empty:
                    logger.info("[INFO] 个股资金流向数据为空")
                    return pd.DataFrame()

                logger.info(f"[OK] 从qstock下载个股资金流向: {len(df)} 只股票")
            ...
            try:
                ...
                if df_filtered.empty:
                    logger.info(f"[INFO] 未找到股票 {stock_code} 的资金流向数据")
                    return pd.DataFrame()
