...
class EasyFactor:
    ...
    def _init_extended_modules(self):
        ...
        try:
            ...
            logger.info("[OK] 资金流向模块已加载")
        except Exception as e:

            logger.info(f"[INFO] 资金流向模块加载失败: {e}")
        ...
        try:
            ...
            logger.info("[OK] 龙虎榜模块已加载")
        except Exception as e:

            logger.info(f"[INFO] 龙虎榜模块加载失败: {e}")
        ...
        try:
            ...
            logger.info("[OK] 板块数据模块已加载")
        except Exception as e:

            logger.info(f"[INFO] 板块数据模块加载失败: {e}")
    ...
    def get_market_data_ex(self,
        ...
        if period != 'daily':
            ...
            logger.warning("[WARNING] DuckDB仅支持日线数据，period参数已忽略")
    ...
    def get_factor(self,
        ...
        try:
            ...
            if factor_name.startswith('momentum_'):
                ...
                logger.warning(f"[WARNING] 未知因子: {factor_name}")
                return pd.DataFrame()
            ...
            logger.error(f"[ERROR] 计算因子失败 {factor_name}: {e}")
            import traceback
        ...
        for factor_name in factor_names:
            ...
            for stock_code in stock_list:
                ...
                try:
                    ...
                    logger.warning(f"[WARNING] 计算{stock_code}的{factor_name}失败: {e}")
                    continue
    ...
    def analyze_batch(self,
        ...
        logger.info(f"[批量分析] 正在读取 {len(stock_list)} 只股票的数据...")
        all_data = self.duckdb_reader.get_market_data(stock_list, start_date, end_date)
    ...
    def get_stock_money_flow(self, stock_code: str, days: int = 5, use_cache: bool = True) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(f"[ERROR] 获取资金流向失败: {e}")
            return pd.DataFrame()
    ...
    def get_sector_money_flow(self, sector_name: str, date: str = None) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(f"[ERROR] 获取板块资金流向失败: {e}")
            return pd.DataFrame()
    ...
    def get_money_flow_rank(self, stock_pool: List[str] = None,
        ...
        try:
            ...
            logger.error(f"[ERROR] 获取资金流向排名失败: {e}")
            return pd.DataFrame()
    ...
    def get_ths_industry_money_flow(self, top_n: int = 20, use_cache: bool = True) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(f"[ERROR] 获取行业资金流向失败: {e}")
            return pd.DataFrame()
    ...
    def get_ths_concept_money_flow(self, top_n: int = 20, use_cache: bool = True) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(f"[ERROR] 获取概念资金流向失败: {e}")
            return pd.DataFrame()
    ...
    def update_ths_money_flow(self) -> Dict[str, int]:
        ...
        try:
            ...
            logger.error(f"[ERROR] 更新同花顺资金流向失败: {e}")
            return {'industry': 0, 'concept': 0}
    ...
    def get_north_money_flow(self, days: int = 30, use_cache: bool = True) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(f"[ERROR] 获取北向资金流向失败: {e}")
            return pd.DataFrame()
    ...
    def get_north_money_sector(self, top_n: int = 20) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(f"[ERROR] 获取北向资金行业流向失败: {e}")
            return pd.DataFrame()
    ...
    def get_north_money_stock(self, stock_code: str = None, top_n: int = 20) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(f"[ERROR] 获取北向资金个股流向失败: {e}")
            return pd.DataFrame()
    ...
    def get_ths_stock_money_flow(self, stock_code: str = None, top_n: int = 20, use_cache: bool = True) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(f"[ERROR] 获取个股资金流向失败: {e}")
            return pd.DataFrame()
    ...
    def get_dragon_tiger_list(self, date: str = None) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(f"[ERROR] 获取龙虎榜失败: {e}")
            return pd.DataFrame()
    ...
    def get_stock_dragon_tiger_history(self, stock_code: str,
        ...
        try:
            ...
            logger.error(f"[ERROR] 获取龙虎榜历史失败: {e}")
            return pd.DataFrame()
    ...
    def get_institutional_detail(self, date: str = None) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(f"[ERROR] 获取机构明细失败: {e}")
            return pd.DataFrame()
    ...
    def get_sector_stocks(self, sector_name: str,
        ...
        try:
            ...
            logger.error(f"[ERROR] 获取板块成分股失败: {e}")
            return pd.DataFrame()
    ...
    def get_sector_rank(self, sector_type: str = 'industry',
        ...
        try:
            ...
            logger.error(f"[ERROR] 获取板块排名失败: {e}")
            return pd.DataFrame()
...
if __name__ == "__main__":
    ...
    logger.info("=" * 90)
    logger.info(" " * 30 + "EasyFactor v3.0 - DuckDB纯化版")
    logger.info("=" * 90)
    ...
    try:
        ...
        logger.info("[OK] EasyFactor初始化成功")
        ...
        if not stocks.empty:
            ...
            logger.info(f"[OK] 数据库中有 {len(stocks)} 只股票")
            logger.info(f"前5只: {stocks['stock_code'][:5].tolist()}")
        ...
        if 'score' in results and not results['score'].empty:
            ...
            logger.info("[OK] 综合评分：")
            logger.info(results['score'].sort_values('score', ascending=False))


    except FileNotFoundError:

        logger.info(f"[INFO] DuckDB数据库不存在: {duckdb_path}")
        logger.info("[提示] 请先创建DuckDB数据库或修改路径")


    logger.info("\n" + "=" * 90)
    logger.info(" " * 35 + "EasyFactor初始化完成")
    logger.info("=" * 90)
