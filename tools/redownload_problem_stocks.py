...
def redownload_stock(stock_code: str, start_date: str = '2020-01-01', end_date: str = '2025-01-31'):
    ...
    try:
        ...
        if not df.empty:
            ...
            logger.warning(f"[WARNING] 下载成功但无数据")
            return False
...
def redownload_all_problem_stocks():
    ...
    if not problem_stocks:
        ...
        logger.info("[OK] 未发现问题股票")
        return



    logger.info(f"发现 {len(problem_stocks)} 只问题股票:")
    for stock in problem_stocks[:10]:  # 只显示前10个

        logger.info(f"  - {stock}")
    if len(problem_stocks) > 10:

        logger.info(f"  ... 还有 {len(problem_stocks) - 10} 只")
    ...
    logger.info("下载完成！")
    logger.info("=" * 80)
    logger.info(f"总计: {len(problem_stocks)} 只")
...
if __name__ == "__main__":
    ...
    if args.list:
        ...
        logger.info("使用方法:")
        logger.info("  列出问题股票: python redownload_problem_stocks.py --list")
        logger.info("  重新下载指定股票: python redownload_problem_stocks.py --stock 000001.SZ")
        logger.info("  重新下载所有问题股票: python redownload_problem_stocks.py --all")
        print()

        logger.info("示例:")
        logger.info("  python redownload_problem_stocks.py --stock 000001.SZ --start 2020-01-01 --end 2025-01-31")
