...
logger.info("=" * 60)
logger.info("方式1: 数据下载 - 使用LocalDataManager")
logger.info("=" * 60)
...
try:
    ...
    logger.info(f"❌ LocalDataManager使用失败: {e}")
    logger.info("提示：请确保101因子平台的相关依赖已安装")
...
logger.info("\n" + "=" * 60)
logger.info("方式2: 数据查询 - 使用easyxt_backtest.DataManager")
logger.info("=" * 60)
...
try:
    ...
    if data is not None and not data.empty:
        ...
        logger.info(f"✓ 查询成功，获取到 {len(data)} 条数据")
        logger.info(data.head())
    else:

        logger.info("⚠️ 查询成功但没有数据（可能是数据库中没有数据）")
    ...
    logger.info(f"❌ DataManager查询失败: {e}")
    logger.info("提示：请确保DuckDB数据库文件存在")
