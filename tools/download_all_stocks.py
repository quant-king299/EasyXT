...
def download_all_stocks(data_dir=None, force_download=False, stocks=None, start_date="", end_date=""):
    ...
    logger.info("-" * 60)
    logger.info("下载完成统计:")
    logger.info(f"  总股票数: {total_count:4d}")
    logger.info(f"  成功下载: {success_count:4d}")
    logger.info(f"  下载失败: {failed_count:4d}")
