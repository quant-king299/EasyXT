...
def update_data():
    ...
    logger.info("=" * 70)
    logger.info("一键补充数据工具")
    logger.info("=" * 70)
    print()

    logger.info("注意：请确保GUI已关闭后再运行此工具")
    print()
    ...
    try:
        ...
        logger.info(f"[OK] 发现 {len(stock_codes)} 只股票需要更新")
        print()
        ...
        for i, stock_code in enumerate(stock_codes):
            ...
            try:
                ...
                logger.info(f"  [{i+1}/{total}] {stock_code}: [错误] {str(e)[:50]}")
                failed_count += 1
        ...
        logger.info(f"  收集完成: {len(update_data)} 条记录")
        print()
        ...
        if update_data:
            ...
            try:
                ...
                logger.info(f"[OK] 成功保存 {len(df_all)} 条记录")
            finally:
        ...
        if failed_list:
            ...
            if len(failed_list) > 10:
                ...
                logger.info(f"  ... 还有 {len(failed_list) - 10} 只")


    except ImportError as e:

        logger.info(f"[错误] 导入失败: {e}")
        logger.info("请确保已安装: pip install pyqt5 duckdb")
    except Exception as e:

        logger.info(f"[错误] 更新失败: {e}")
        import traceback
