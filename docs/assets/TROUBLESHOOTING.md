...
- **方式A**：有 Tushare Token（推荐，注册 https://tushare.pro/weborder/#/login?reg=344724 并建议充值到 2000 积分以上）— 不需要 QMT
...
1. 访问 https://tushare.pro/weborder/#/login?reg=344724 ，点击右上角「注册」
2. 用手机号注册（免费，注册即送积分）
...
**问题**：有用户反馈大 QMT 下载数据占用空间超大，是否真的如此？

**答案**：大 QMT 数据确实比 miniQMT 占用更多空间，但这实际上是**优势而非问题**。

##### 文件大小对比

以典型股票为例：

| 股票代码 | 大QMT | miniQMT | 差异 | 上市年份 |
|---------|-------|---------|------|----------|
| 000001.SZ | 528 KB | 246 KB | +282 KB | 1991 |
| 000001.SH | 542 KB | 42 KB | +500 KB | 1990 |
| 000002.SZ | 527 KB | 152 KB | +375 KB | 1991 |
| 600519.SH | 372 KB | 184 KB | +188 KB | 2001 |
| 600000.SH | 396 KB | 177 KB | +219 KB | 2000 |

##### 根本原因：历史数据覆盖范围不同

大 QMT 的优势在于包含了**更早的历史数据**：

| 上市年份 | 大QMT起始 | miniQMT起始 | 记录数差异 | 说明 |
|---------|-----------|------------|-----------|------|
| 1991年 | 1991-01-05 | 2009-2015 | +9000~12000条 | 深市"老八股" |
| 2000-2006 | 上市首日 | 2014-2015 | +4000~7000条 | 早期蓝筹股 |
| 2018年后 | 上市首日 | 上市首日 | +8条 | 新股（约0.2KB） |

- **老股票（1990s-2000s 上市）**：大 QMT 从上市首日开始，miniQMT 从 2014-2015 年开始，差异巨大
- **新股票（2018 后上市）**：两边都从上市首日开始，差异仅约 8 条记录（约 0.2 KB）

##### 数据起始日期分布

通过分析 100 只股票的起始日期：

- **miniQMT**：起始日期分散在 2009-2025 之间，最早从 2009 年 12 月开始
- **大 QMT**：起始日期集中在 1990-2009 之间，有完整的 1990 年代数据
...
- 每条记录约 32 字节
- 单只股票约 480 KB
- **总计约 2.4-4 GB**（含索引和 overhead）

##### 导入 DuckDB 后的空间占用

大 QMT 导入 DuckDB 后同样会占用更多空间，但这是合理的数据存储：

- DuckDB 的列式存储和压缩可以有效优化空间占用
- 包含了更有价值的历史信息（1990年代数据）
- 对于需要长期历史分析的场景（如回测 2000 年代的策略），这些数据是必需的

##### 总结

| 特性 | 说明 |
|------|------|
| **占用空间大** | ✅ 这是优势，包含了更完整的历史数据 |
| **老股票差异大** | 1990s-2000s 上市的股票多了十几年历史数据 |
| **新股票差异小** | 2018 年后上市的股票差异小于 1KB |
| **适用场景** | 需要长期历史回测、早期数据研究的用户 |

**回复粉丝建议**：
> 大 QMT 文件大是因为包含更早的历史数据（2000年代上市的股票多了 4000-7000 条记录，90年代上市的甚至多了 1 万多条）。2018 年后上市的股票两边基本一致，差异小于 1KB。按 5,000 只股票全日线算，DuckDB 约 3-4 GB，不算大。

#### QMT 数据是否加密？

**问题**：下载的数据是加密的吗？

**答案**：**不是加密的**，而是使用**明文二进制格式**存储，完全可以自由读取和使用。

##### 数据存储格式

QMT 的数据文件（.DAT）使用简单的二进制结构存储，没有任何加密：
...
# 读取日线文件
with open(r'D:\国金QMT交易端模拟\datadir\SH\86400\600000.SH.DAT', 'rb') as f:
    data = f.read()
...
# 解析每条记录
for i in range(0, (len(data) - header_size) // rec_size, 2):  # 步长为2（奇数索引是复权元数据）
...
| 操作 | 说明 |
|------|------|
| **备份** | 直接复制 `datadir` 文件夹到其他位置 |
| **迁移** | 复制到另一台电脑继续使用 |
| **分析** | 用任何工具读取二进制文件 |
| **转换** | 转换为 CSV、Excel、数据库等格式 |
| **共享** | 在自己的设备间自由传输 |
...
con = duckdb.connect('D:/StockData/stock_data.ddb', read_only=True)
df = con.execute("SELECT * FROM stock_daily WHERE stock_code = '000001.SZ' LIMIT 10").fetchdf()
print(df)
```

DuckDB 也是**开源格式**，同样不是加密的：
- 可以用任何支持 DuckDB 的工具读取
- 可以导出为 CSV、Parquet 等格式
- 完全属于用户，可自由处理

**回复粉丝建议**：
> QMT 下载的数据**不是加密的**，而是明文二进制格式存储。你可以直接复制 `datadir` 文件夹备份，或者用 EasyXT 的 `QMTLocalReader` 读取，甚至自己写代码解析二进制文件。数据完全属于你，可以自由使用、备份、迁移。导入 DuckDB 后也是开源格式，同样可自由读取。
...
**步骤 1**：获取 Token
- 访问 https://tushare.pro/weborder/#/login?reg=344724 注册
- 登录后进入「用户中心」→「接口Token」
