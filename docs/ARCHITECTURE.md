...
回测系统按资产类别分两路，CB 无需复权走向量化引擎，ETF 和股票走增强引擎 + adj_factor 表本地计算：
...
| `easyxt_backtest/vectorized_engine.py` | **新增**：CB 向量化回测引擎（不复权），ETF 支持 adj_factor 复权 |
...
**CB（向量化引擎，无需复权）/ ETF（支持 adj_factor 复权）**：
```python
