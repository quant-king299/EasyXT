...
class FamaFrenchCalculator:
    ...
    def _calculate_umd_factor(self, stock_data: pd.DataFrame,
        ...
        try:
            ...
            logger.warning(f"[WARNING] 计算UMD因子失败: {e}")
            return np.nan
    ...
    def _get_stock_data(self, trade_date: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if hasattr(self.data_manager, 'get_market_data'):
                ...
                logger.warning("[WARNING] data_manager不支持获取数据的方法")
                return None
...
if __name__ == "__main__":
    ...
    logger.info("=" * 70)
    logger.info(" " * 20 + "Fama-French定价因子计算器")
    logger.info("=" * 70)


    logger.info("\n[功能说明]")
    logger.info("- MKT（市场因子）：所有股票的平均收益")
    logger.info("- SMB（规模因子）：小市值组合收益 - 大市值组合收益")
    logger.info("- HML（价值因子）：低PB组合收益 - 高PB组合收益")
    logger.info("- UMD（动量因子）：高收益组合 - 低收益组合")
