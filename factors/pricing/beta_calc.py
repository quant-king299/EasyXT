...
class BetaCalculator:
    ...
    def calculate_stock_beta(self,
        ...
        try:
            ...
            logger.warning(f"[WARNING] {stock_code} Beta计算失败: {e}")
            return {
    ...
    def _get_stock_returns(self,
        ...
        try:
            ...
            logger.warning(f"[WARNING] 获取{stock_code}收益率失败: {e}")
            return None
    ...
    def _get_pricing_factors(self,
        ...
        try:
            ...
            logger.warning(f"[WARNING] 获取定价因子失败: {e}")
            return None
...
if __name__ == "__main__":
    ...
    logger.info("=" * 70)
    logger.info(" " * 25 + "Beta系数计算器")
    logger.info("=" * 70)


    logger.info("\n[功能说明]")
    logger.info("- beta_mkt: 对市场因子的敏感度（>1表示进攻型，<1表示防御型）")
    logger.info("- beta_smb: 对规模因子的敏感度（>0表示偏向小盘股）")
    logger.info("- beta_hml: 对价值因子的敏感度（>0表示偏向价值股）")
    logger.info("- alpha: 回归截距（超额收益）")
    logger.info("- r_squared: 拟合优度（越接近1解释力越强）")
    ...
    logger.info("\n[依赖]")
    logger.info("- statsmodels（推荐）：完整的回归统计")
    logger.info("- scipy（备选）：简化的回归计算")
