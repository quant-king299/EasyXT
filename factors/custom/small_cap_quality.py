...
class SmallCapQualityFactor:
    ...
    def calculate(self,
        ...
        if stock_data is None or stock_data.empty:
            ...
            logger.info(f"[INFO] {trade_date} 未获取到股票数据")
            return pd.DataFrame()
        ...
        if stock_data.empty:
            ...
            logger.info(f"[INFO] {trade_date} 过滤后无股票")
            return pd.DataFrame()
        ...
        if financial_data is None or financial_data.empty:
            ...
            logger.info(f"[INFO] {trade_date} 未获取到财务数据")
            return pd.DataFrame()
        ...
        if qualified_stocks.empty:
            ...
            logger.info(f"[INFO] {trade_date} 基本面筛选后无股票")
            return pd.DataFrame()
        ...
        try:
            ...
            if hasattr(data_manager, 'get_market_data'):
                ...
                if not all(col in df.columns for col in required_cols):
                    ...
                    logger.warning(f"[WARNING] 数据缺少必要列: {required_cols}")
                    return None
                ...
                logger.warning("[WARNING] data_manager不支持获取股票数据")
                return None
            ...
            logger.warning(f"[WARNING] 获取股票数据失败: {e}")
            return None
    ...
    def _get_financial_data(self,
        ...
        try:
            ...
            logger.info(f"[INFO] 获取财务数据失败（可选）: {e}")
            return None
    ...
    def batch_calculate(self,
        ...
        try:
            ...
            logger.warning(f"[WARNING] 获取交易日历失败: {e}")
            return pd.DataFrame()
        ...
        for trade_date in trade_dates:
            ...
            try:
                ...
                logger.warning(f"[WARNING] {trade_date} 因子计算失败: {e}")
                continue
...
if __name__ == "__main__":
    ...
    logger.info("=" * 70)
    logger.info(" " * 15 + "小市值质量因子（f1）")
    logger.info("=" * 70)


    logger.info("\n[因子逻辑]")
    logger.info("1. 股票池过滤：")
    logger.info("   - 剔除创业板、科创板、北交所")
    logger.info("   - 剔除ST股票")
    logger.info("   - 剔除停牌、高价股（>100元）")


    logger.info("\n2. 基本面筛选：")
    logger.info("   - ROE > 15%（高盈利能力）")
    logger.info("   - ROA > 10%（高资产回报率）")


    logger.info("\n3. 因子计算：")
    logger.info("   - rank_mv = 市值排名（百分比）")
    logger.info("   - rank_pb = PB排名（百分比）")
    logger.info("   - factor_value = -(rank_mv + rank_pb) / 2")
    logger.info("   - 因子值越小越好（小市值+低PB）")
    ...
    logger.info("\n[因子的投资逻辑]")
    logger.info("- 小市值：小盘股通常有更大的成长空间")
    logger.info("- 高质量：高ROE/ROA表示公司盈利能力强")
    logger.info("- 低估值：低PB表示被低估，安全边际高")
    logger.info("- 组合：小盘+优质+低估 = 高性价比")
