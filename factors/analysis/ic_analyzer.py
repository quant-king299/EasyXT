...
class ICAnalyzer:
    ...
    def calculate_ic(self,
        ...
        if len(common_dates) == 0:
            ...
            logger.warning("[WARNING] factor_df和return_df没有共同的日期")
            return pd.Series(dtype=float)
    ...
    def _to_wide_format(self,
        ...
        try:
            ...
            logger.warning(f"[WARNING] 转换为wide格式失败: {e}")
            return None
...
if __name__ == "__main__":
    ...
    logger.info("=" * 70)
    logger.info(" " * 25 + "IC分析器")
    logger.info("=" * 70)


    logger.info("\n[功能说明]")
    logger.info("- IC（Information Coefficient）：因子与收益率的相关系数")
    logger.info("- Rank IC：Spearman秩相关系数（常用，更稳健）")
    logger.info("- Normal IC：Pearson线性相关系数")
    logger.info("- IR（Information Ratio）：IC均值 / IC标准差")
    logger.info("- IC半衰期：IC衰减到一半所需天数")


    logger.info("\n[IC评价标准]")
    logger.info("  |IC| > 0.05 : 强预测能力")
    logger.info("  |IC| > 0.03 : 较强预测能力")
    logger.info("  |IC| > 0.02 : 有一定预测能力")
    logger.info("  |IC| < 0.02 : 预测能力较弱")


    logger.info("\n[IR评价标准]")
    logger.info("  IR > 1.0 : 优秀")
    logger.info("  IR > 0.5 : 良好")
    logger.info("  IR > 0.3 : 一般")
    logger.info("  IR < 0.3 : 较差")
    ...
    logger.info(f"IR: {ic_stats['ir']:.4f}")
    logger.info(f"t统计量: {ic_stats['t_stat']:.4f}")
    ...
    logger.info(f"IC半衰期: {half_life['half_life']:.0f}天")
    """)
