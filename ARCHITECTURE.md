...
- ✅ AdjustmentCache 补充 tushare 兜底（QMT 离线时自动降级）
...
### E. 复权数据架构（2026-06-04 新增）

#### E.1 整体架构图

项目中有**两套数据管道**，分别服务 GUI 和回测引擎，复权处理统一走 `AdjustmentCache`：
...
│                                                                     │
│   GUI 数据查看器                回测引擎                              │
│   (advanced_data_viewer)       (enhanced_backtest_engine)           │
│         │                            │                              │
│         ↓                            ↓                              │
│   UnifiedDataInterface         UnifiedDataInterface                │
│   (data_manager/)              (data_manager/)                     │
│         │                            │   adjust='back'              │
│         ↓                            ↓                              │
│   AdjustmentCache ──────────── AdjustmentCache                     │
│   (data_manager/adjustment_cache.py)                                │
│         │                                                           │
│         ├─ 1. QMT 在线 (主力方案，准确率 100%)                        │
│         │     xtdata.get_market_data_ex(dividend_type='back')       │
│         │                                                           │
│         ├─ 2. tushare adj_factor (方案B，QMT 离线时)                  │
│         │     pro.adj_factor() + pro.daily()                        │
│         │     adj_price = close × (factor_today / factor_latest)    │
│         │                                                           │
│         ├─ 3. tushare pro_bar (方案A，方案B也失败时)                  │
│         │     ts.pro_bar(adj='hfq')  直接获取复权数据                │
│         │                                                           │
│         └─ 4. 全部失败 → DuckDB 原始数据 + warning                   │
│                                                                     │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│   回测引擎的备选路径 (fallback)：                                     │
│   UnifiedDataInterface 不可用时 → HybridDataManager                  │
│   (core/data_manager/) → 不复权数据                                  │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
| `data_manager/unified_data_interface.py` | 统一数据接口：DuckDB + QMT + 复权调度 |
| `easyxt_backtest/enhanced_backtest_engine.py` | 回测引擎：`adjust='back'` 参数，价格获取走复权接口 |
| `gui_app/widgets/advanced_data_viewer.py` | GUI 数据查看器：已有复权切换（前/后/等比） |
| `core/data_manager/sources/tushare_source.py` | tushare 数据源（HybridDataManager 用，不含复权） |
| `core/data_manager/hybrid_manager.py` | 混合数据管理器（回测引擎 fallback 用） |
...
engine = EnhancedBacktestEngine(
    ...
    data_manager=data_manager,
    adjust='back'   # 默认后复权，可选 'none', 'front', 'back'
)
```

内部流程：
1. `_get_current_prices()` → 优先调 `UnifiedDataInterface.get_stock_data(adjust='back')`
2. `_load_daily_prices()` → 同上，批量获取复权数据
3. 复权接口不可用时 → fallback 到原有 `HybridDataManager`（不复权）
